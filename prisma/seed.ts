...
async function main() {
  ...
  const owner1 = await prisma.owner.upsert({
    ...
    update: {},
    create: { name: "דוד כהן", phone: "050-1234567", email: "david@example.com", idNumber: "123456789" },
  });

  const owner2 = await prisma.owner.upsert({
    where: { idNumber: "987654321" },
    update: {},
    create: { name: "שרה לוי", phone: "052-9876543", email: "sarah@example.com", idNumber: "987654321" },
  });

  const existingProps = await prisma.property.count();
  if (existingProps === 0) {
    let building = await prisma.building.findFirst({ where: { address: "גבעת שאול 12" } });
    if (!building) {
      building = await prisma.building.create({
        data: { name: "בניין גבעת שאול 12", address: "גבעת שאול 12", city: "תל אביב", floors: 8, units: 24 },
      });
    }
    ...
    const existingTenant = await prisma.tenant.findFirst({ where: { idNumber: "111111111" } });
    ...
    if (rentedProp) {
      const existingLease = await prisma.lease.findFirst({ where: { propertyId: rentedProp.id } });
      if (!existingLease) {
        await prisma.lease.create({
          data: { propertyId: rentedProp.id, tenantId: tenant.id, startDate: new Date("2024-07-01"), endDate: new Date("2025-06-30"), monthlyRent: 6500, deposit: 13000, status: "ACTIVE" },
        });
      }
    }

    const existingLead = await prisma.lead.findFirst({ where: { phone: "053-2222222" } });
    if (!existingLead) {
      await prisma.lead.create({
        data: {
          name: "נועה אברהם", phone: "053-2222222", city: "תל אביב", neighborhood: "הצפון",
          minRooms: 3, maxRooms: 4, minBudget: 7000, maxBudget: 9000, requireParking: true,
          rawText: "שלום, אני מחפשת דירת 3-4 חדרים בצפון תל אביב, תקציב 7000-9000 ש״ח לחודש, חייבת חניה.",
          source: "WHATSAPP", aiSummary: "לקוחה מחפשת דירת 3-4 חדרים בצפון תל אביב עם חניה, תקציב 7,000-9,000 ש״ח.",
          aiQuestions: ["מהו תאריך הכניסה המבוקש?", "האם נדרשת מעלית?", "האם מרוהטת?"], status: "NEW",
        },
      });
    }
  }
